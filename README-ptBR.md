# 👋 Augustto Santana

Senior Java Developer | Backend Specialist | Tech Lead

![Java](https://img.shields.io/badge/Java-21+-ED8B00?style=flat&logo=openjdk&logoColor=white)
![Spring](https://img.shields.io/badge/Spring-Boot-6DB33F?style=flat&logo=spring&logoColor=white)
![Kafka](https://img.shields.io/badge/Apache-Kafka-231F20?style=flat&logo=apachekafka&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat&logo=kubernetes&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-Cloud_Practitioner-FF9900?style=flat&logo=amazonaws&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-AZ900%20%7C%20DP900-0078D4?style=flat&logo=microsoftazure&logoColor=white)

## Sobre Mim

Profissional com mais de 15 anos de experiência em Tecnologia da Informação atuando no desenvolvimento, sustentação e evolução de sistemas corporativos para os segmentos de Telecom e Banking.

Ao longo da carreira participei da construção e manutenção de aplicações críticas responsáveis pelo processamento de grandes volumes de dados, integração entre sistemas corporativos, atendimento a requisitos regulatórios e suporte a aplicações utilizadas diariamente por milhares de usuários.

Possuo experiência em todas as etapas do ciclo de desenvolvimento de software, desde levantamento de requisitos e definição técnica até implantação, monitoramento e sustentação em produção.

Também atuei como líder técnico de equipes de desenvolvimento, realizando code reviews, pair programming, troubleshooting assistido e disseminação de boas práticas de engenharia de software.

---

## Especialidades

* Desenvolvimento Backend
* APIs REST
* Microsserviços
* Integração entre Sistemas
* Modernização de Legados
* Arquitetura de Software
* Sistemas de Missão Crítica
* Troubleshooting e Sustentação
* Liderança Técnica

---

## Principais Tecnologias

### Backend

* Java 7, 8, 11, 17 e 21
* Kotlin
* Spring Boot
* Spring Cloud
* Spring Security
* Spring Batch
* Hibernate / JPA

### Integração e Mensageria

* Kafka
* RabbitMQ
* JMS
* SOAP
* REST APIs

### Bancos de Dados

* PostgreSQL
* Oracle Database
* SQL Server
* MongoDB
* Redis

### DevOps e Cloud

* Docker
* Kubernetes
* Jenkins
* GitLab CI/CD
* Azure DevOps

### Plataformas Cloud

* AWS
* Microsoft Azure
* Oracle Cloud

---

## Arquitetura e Engenharia de Software

* Hexagonal Architecture
* Clean Architecture
* Domain-Driven Design (DDD)
* Event-Driven Architecture
* Backend for Frontend (BFF)
* Design Patterns
* SOLID
* Clean Code

---

## Certificações

* AWS Certified Cloud Practitioner
* Microsoft Azure Fundamentals (AZ-900)
* Microsoft Azure Data Fundamentals (DP-900)
* Scrum Foundation Professional Certificate (SFPC)
* ITIL v3 Foundation

---

## Projetos em Destaque

Os repositórios destacados neste perfil possuem como objetivo demonstrar conhecimentos técnicos, arquiteturas e práticas de engenharia de software utilizadas ao longo da minha trajetória profissional.

Projetos atualmente em desenvolvimento:

* Event Driven Commerce
* Receivables API
* OAuth2 Resource Server
* Spring Batch Processor
* Observability Lab

---

## Nota sobre os Projetos

Grande parte da minha experiência profissional foi construída em projetos corporativos para empresas dos segmentos de Telecom, Banking e Serviços Financeiros.

Por questões de confidencialidade, propriedade intelectual e acordos contratuais, os códigos-fonte e artefatos desenvolvidos nesses projetos não podem ser disponibilizados publicamente.

Os repositórios presentes neste perfil foram criados com o objetivo de demonstrar conhecimentos técnicos, arquiteturas, padrões de projeto e práticas de engenharia de software aplicados ao longo da minha trajetória profissional.

---

## Contato

* LinkedIn: https://www.linkedin.com/in/augusttosantana
* GitHub: https://github.com/augusttosantana
